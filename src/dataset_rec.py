            ...
            for line in tqdm(lines):
                ...
                for item in dialog['rec']:
                    data = {
                        ...
                        'node_list': dialog['node_list'],
                        'edges': dialog['edges'],
                        'adj_shape': dialog['adj_shape'],

                        'seeker_id': dialog['seeker_id']
                    }
...
class CRSRecDataCollator:
    ...
    def __call__(self, data_batch):
        ...
        for data in data_batch:
            ...
            node_list_batch.append(data['node_list'])
            edges_batch.append(data['edges'])
            shape_batch.append(data['adj_shape'])

            seeker_id_batch.append(data['seeker_id'])
        ...
        node_list_batch = padded_tensor(node_list_batch, pad_idx=self.pad_entity_id, pad_tail=True, device=self.device)
        edges_batch = padded_tensor(edges_batch, pad_idx=self.pad_entity_id, pad_tail=True, device=self.device)
        shape_batch = torch.as_tensor(shape_batch, device=self.device)

        input_batch['node_list'] = node_list_batch
        input_batch['edges'] = edges_batch
        input_batch['shape'] = shape_batch

        input_batch['seeker_id'] = seeker_id_batch
        return input_batch
...
if __name__ == '__main__':
    ...
    model_name_or_path = r"E:\Lh-code\huggingface\DialoGPT-small"
    tokenizer = AutoTokenizer.from_pretrained(model_name_or_path)
    tokenizer.add_special_tokens(gpt2_special_tokens_dict)
    prompt_tokenizer = AutoTokenizer.from_pretrained(r'E:\Lh-code\huggingface\roberta-base')
